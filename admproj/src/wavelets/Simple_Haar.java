...
public class Simple_Haar implements IWavelet {
	 ...
	 * 
	 <p>
	 * Calculate the Haar wavelet transform (the ordered fast Haar wavelet
	 * tranform). This calculation is <u>not</u> done in place.
	 * 
	 * <p>
	 * 
	 * @param values
	 *            a <tt>values</tt>: an array of double values on which the Haar
	 ...
	 *
	 <p>
	 * Calculate the inverse haar transform from the coefficients and the Haar
	 * value.
	 * <p>
	 * The inverse function will overwrite the original data that was used to
	 * calculate the Haar transform. Since this data is initialized by the
	 * caller, the caller should make a copy if the data should not be
	 * overwritten.
	 * <p>
	 * The coefficients are stored in in a Java <tt>Vector</tt> container. The
	 * length of the coefficient arrays is ordered in powers of two (e.g., 1, 2,
	 * 4, 8...). The inverse Haar function is calculated using a butterfly
	 * pattern to write into the data array. An initial step writes the Haar
	 * value into data[0]. In the case of the example below this would be
	 * 
	 * <pre>
	 * data[0] = 5.0;
	 * </pre>
	 * <p>
	 * Then a butterfly pattern is shown below. Arrays indices start at 0, so in
	 * this example <tt>c[1,1] is the second element of the
...
d[7] = tmp - c[2, 3]
	...
	public void inverse() {
		if (data != null && coef != null && coef.size() > 0) {
			...
			if (len > 0) {
				...
				len = binary.power2(log); // calculation must be on 2 ** n
											// values
	...
	/**
	 * Print the simple Haar object (e.g, the final Haar step value and the
	 * coefficients.
	 */
	public void pr() {
		if (coef != null) {
	 ...
	 * 
	 * <p>
	 * The <tt>pr()</tt> method prints the coefficients in increasing frequency.
	 * This function prints the data values which were used to generate the Haar
	 * transform.
	 */
