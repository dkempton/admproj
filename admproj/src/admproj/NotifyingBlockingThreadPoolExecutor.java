 ...
 * 
 * 1) The execute method of the ThreadPoolExecutor will block in case the queue
 * is full and only unblock when the queue is dequeued - that is a task that is
 * currently in the queue is removed and handled by the ThreadPoolExecutor.
 * 
 * 2) Client code can await for the event of all tasks beeing run to conclusion.
 * Client code which actively chose to wait for this occurrence should call
 * await on the instance of his ThreadPoolExecutor. This differs from
 * awaitTermination as it does not require any call to shutdown.
 * 
 ...
 * 
 * @author (Original) Yaneeve Shekel & Amir Kirsh
 * @author (Customized by) Dustin Kempton
 **/
public class NotifyingBlockingThreadPoolExecutor extends ThreadPoolExecutor {
	 ...
	 */
	public NotifyingBlockingThreadPoolExecutor(int poolSize, int queueSize,
			long keepAliveTime, TimeUnit keepAliveTimeUnit,
			long maxBlockingTime, TimeUnit maxBlockingTimeUnit,
		...
		super(
				...
				keepAliveTime,
				keepAliveTimeUnit,
				new ArrayBlockingQueue<Runnable>(Math.max(poolSize, queueSize)), // not
																					// smaller
																					// than
																					// the
																					// poolSize
																					...
																					// redundant
																					// threads)
				new BlockThenRunPolicy(maxBlockingTime, maxBlockingTimeUnit,
						blockingTimeCallback)); // When super invokes the reject
												// method this class will ensure
												// a blocking try
	...
	public NotifyingBlockingThreadPoolExecutor(int poolSize, int queueSize,
		...
		super(poolSize, // Core size
				poolSize, // Max size
				keepAliveTime, unit, new ArrayBlockingQueue<Runnable>(Math.max(
						poolSize, queueSize)), // not smaller than the poolSize
												// (to avoid redundant threads)
				new BlockThenRunPolicy()); // When super invokes the reject
											// method this class will ensure a
											// blocking try.
	...
	public void setMaximumPoolSize(int maximumPoolSize) {
		throw new UnsupportedOperationException(
				"setMaximumPoolSize is not supported.");
	}
	...
	/**
	 * A blocking wait for this ThreadPool to complete "A TASK" or a certain
	 * timeout to elapse. Similar to await() method, except for
	 * adding the timeout condition and only needs one task to complete
	 * before a thread waiting will be notified.
	 * 
	 * @see NotifyingBlockingThreadPoolExecutor#await() for more details.
	 * @return false if the timeout elapsed, true if the synch event we are
	 *         waiting for had happened.
	 ...
	 */
	public boolean awaitOne(long timeout, TimeUnit timeUnit) throws InterruptedException{
	...
	private class Synchronizer {
		...
		private final Condition oneDone = lock.newCondition();
		...
		/**
		 * This PRIVATE method allows the ThreadPoolExecutor to notify one
		 * interested party that a task given to the execute method has run to
		 * conclusion and can its results are now ready.
		 **/
		private void signalOne() {

			lock.lock(); // MUST lock!
			try {
				oneIsDone = true; // To help the awaitOne method ascertain that
									// it has
									// not woken up 'spuriously'
				oneDone.signal();
			} finally {
				lock.unlock(); // Make sure to unlock even in case of an
								// exception
		...
		/**
		 * This is the inner implementation for supporting the
		 * NotifyingBlockingThreadPoolExecutor.awaitOne(timeout, timeUnit).
		 * 
		 * @see NotifyingBlockingThreadPoolExecutor#awaitOne(long, TimeUnit) for
		 *      details.
		 * @throws InterruptedException
		 *             when the internal condition throws it.
		 * @author Dustin Kempton
		 */
		public boolean awaitOne(long timeout, TimeUnit timeUnit)
				throws InterruptedException {
			boolean await_result = false;
			lock.lock(); // Must lock!
			boolean localIsDone;
			try {
				await_result = oneDone.await(timeout, timeUnit);
			} finally {

				localIsDone = oneIsDone;
				oneIsDone = false; // for next time
				lock.unlock(); // Make sure to unlock even in case of an
								// exception
			}
			return await_result && localIsDone;
	 ...
	 * NotifyingBlockingThreadPoolExecutor. It does so by invoking the
	 * BlockingQueue's put method (instead of the offer method that is used by
	 * the standard implementation of the ThreadPoolExecutor - see the opened
	 * Java 6 source code).
	 */
			...
			while (!taskSent) {
				...
				try {
					...
					if (blockingTimeCallback != null) {
						...
						// was accepted or not
						if (workQueue.offer(task, maxBlockingTime,
								maxBlockingTimeUnit)) {
							// task accepted
							taskSent = true;
						} else {
